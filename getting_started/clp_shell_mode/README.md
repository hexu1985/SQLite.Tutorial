### Shell模式下的CLP（Command Line Program）

要触发CLP的Shell模式，可在命令行输入sqlite3，命令后跟随一个可选的数据库文件名。如果在命令行中不指定数据库名，SQLite将会使用一个内存数据库（其内容推出CLP时将会丢失）。
...
3           miny
4           mo
Run Time: real 0.000 user 0.000331 sys 0.000000
```
